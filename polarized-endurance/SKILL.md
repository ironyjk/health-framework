...
- **Endurance goals**: marathon·ultra·gran fondo·triathlon·cross-country riding
- **FTP·VO2max plateau** — when stuck in moderate intensity
...
### Recreational Cyclist, 6~8h/week
```
Mon: Rest
Tue: Z2 60~90min
Wed: VO2 intervals 4~5 x 4min @Z5 / 3min recov (60min total)
Thu: Z2 60min
Fri: Rest
Sat: Long Z2 2~4h
Sun: Z2 60~90min + optional short Z4 blocks
```
→ Low intensity ~80%, high intensity ~15~20%

### Runner 40~60 km/week
```
Easy pace runs 3~4 times (conversational)
VO2 session 1x/week (400m x 8, 1km x 5~6)
Long run 1x/week (Z2)
Threshold run 1~2x/month or skip
```

### VO2max Interval Recipes
- **4x4min @Z5** / 3min recovery (Norwegian 4x4, best researched)
- **5x3min** / 3min
- **30/15** (Rønnestad session) — 13x30sec max / 15sec Z1, 3 sets
- **40/20** — beginners

### How to Identify Z2
- Maximum intensity where **nasal breathing is still possible**
- **Talk test**: complete sentences still possible
...
- FTP or 10k pace test every 4~8 weeks
- HRV·resting HR (links with `recovery-periodization`)
- **VO2max percentile** (age·sex normed) — high risk <20th, average 40~60th, excellent >80th. Strongest predictor of all-cause mortality (Mandsager 2018)

### Estimated VO2max (age·sex "good" range, ml/kg/min)
| Age | Men (good) | Women (good) |
|---|---|---|
| 30s | 44~51 | 38~44 |
| 40s | 41~47 | 35~40 |
| 50s | 37~43 | 32~37 |
...
- **Indoor trainers (Zwift·Wahoo·Tacx)** — vibration·noise issues with Korean housing (apartments). Balcony·separate space·rubber mat. Advantageous for maintaining structured Z2
- **Gran fondo·cross-country riding** — long-duration Z2 sustainability is key. Separately manage 5~8h saddle adaptation through training + equipment (saddle·bibs)
...
- **Daily Z3 "gray zone"** — always tired yet not improving
- **Doing Z2 too fast** — without HR measurement, "comfortable" standard drifts into Z3
- **VO2 sessions 2x+/week long-term** — accumulated fatigue, immune suppression
- **Long ride Z3 drift every weekend** — starts Z2 but ends Z3
- **Matching 80/20 ratio within a single session** — this is about weekly·block-level distribution
- **Skipping strength training** — endurance alone insufficient for sarcopenia prevention. Resistance exercise recommended in parallel
...
2. **Large individual VO2max variation** — responder spectrum
3. **Sport specificity** — low-intensity running volume creates joint burden → runners need cross-training
4. **Adjust high-intensity sessions 2x → 1x/week** based on age·recovery capacity
...
- Strength in parallel → `strength-basics` (minimum 2x/week lower body)
